...
```{r}
b <- findFn("{ordered factor}")
findFn("ordered response factor")
...
Cumulative number of downloads of all the selected packages from 11 November 2013 to 11 November 2023
...
packages <- c("brms", "VGAM", "MachineShop", "rms", "multgee", "tram", 
              "glmmLasso", "MPDiR", "ordinalgmifs", "MASS", 
...
df_downloads_2 <- df_downloads_grouped %>%
  filter(package %in% c("arm","VGAM","rms","ordinal","brms"))
...
df_downloads_4 <- df_downloads_grouped %>%
  filter(!(package %in% c("MASS","arm","VGAM","rms","ordinal","brms",
...
ndownload_packages <- c("MASS","arm","rms","VGAM","ordinal","brms",
                        "tram","multgee","MXM","ordinalCont","glmmLasso",
                        "lordif","ordinalNet","MPDiR","MachineShop","oglmx",
                        "mvord","OrdFacReg","gnlm","ordinalgmifs",
                        "jrt","BayesOrdDesign","ordinalbayes")

custom_colors <- c("red2","gold1","mediumpurple","palegreen2","hotpink",
                   "sienna","steelblue1","orange","palegreen4",
                   "mediumorchid1","tan","tomato3")
...
ggplot(df_downloads_1, aes(x = year_month, y = ac_count, color = factor(package, levels = ndownload_packages))) +
  ...
  theme_minimal() +
  xlim(as.Date("2013-11-01"),as.Date("2023-10-01")) +
...
for(year in 2009:2023){
  for(i in 1:ncol(Update_year)){
    for(j in 1:nrow(na.omit(Update_year[,i]))){
      if(Update_year[j,i] == year){
        count <- count + 1
      } else {
        count <- count
      }
...
ggplot(updates, aes(x = Year, y = Count)) +
